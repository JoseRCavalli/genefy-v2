...
### Routing: Next App Router as a thin shell over a CSR SPA

`src/app/` holds the routes (`/`, `/login`, `/solicitar`, `/app`, plus a `[...rest]` catch-all that redirects to `/`). Every `page.tsx` is a `'use client'` wrapper that loads the real screen from `src/views/` via `next/dynamic` with `ssr: false` — the app is deliberately 100% CSR (identical to its pre-Next behavior; several components read `localStorage` in `useState` initializers, so do NOT remove `ssr: false` or convert anything to Server Components without handling that). `ssr: false` is only legal inside Client Components — keep the `'use client'` directive on those pages. Providers (AuthContext) are mounted in `src/app/providers.tsx`.
...
Separately, inside `SupabaseApp` there is a **demo account**: logging in as `demo@gmail.com` short-circuits auth in `src/contexts/AuthContext.tsx` (mock session in localStorage, no Supabase Auth call), `useFarm` creates/loads a "Fazenda Teste" farm, and `useFemales` serves `DEMO_FEMALES` (100 fictional females generated by a seeded PRNG in `src/lib/demo-females.ts` — deliberately fake; must never show real herd data). Do not confuse this account with `NEXT_PUBLIC_DEMO_MODE`.

**Consequence:** any new data feature must be implemented twice — in the Supabase hook AND in the matching `useDemo*` hook (same return shape), and often given a demo-account branch as well.
...
Hooks load rows and convert (`rowToFemale` in useFemales.ts); both forms (`bulls`+`bullRows`, `females`+`femaleRows`) are threaded through props side by side. When adding a column you typically touch: SQL migration, the Row interface, the domain interface, the row→domain mapper, and the demo hook mapper.
...
The full females × bulls matrix (Análise Completa tab) is computed in a Web Worker: `src/workers/matrixWorker.ts`.
